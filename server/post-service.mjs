...
function letterSnapshot(input) {
  ...
  if (!STAMPS.some(stamp => stamp.id === input.stampId)) throw fail("请选一枚邮票");
  return {
...
export function createPostService({
  ...
  async function loginResponse(request, user) {
    const token = randomBytes(32).toString("hex");
    const expiresAt = clock() + SESSION_LIFE;
    await run("DELETE FROM sessions WHERE expires_at<=?", clock());
    await run(
    ...
    if (bearerSessions)
      return json({ user: await publicUser(user), session: { token, expiresAt } });
    return json({ user: await publicUser(user) }, 200, {
      "Set-Cookie": cookie(request, token),
    });
  }
  async function throttle(key, limit = 20) {
    const now = clock();
    // One statement prevents concurrent attempts from bypassing the limit.
    const entry = await one(
      "INSERT INTO attempts VALUES(?,1,?) ON CONFLICT(key) DO UPDATE SET count=CASE WHEN resets_at<=? THEN 1 ELSE count+1 END,resets_at=CASE WHEN resets_at<=? THEN ? ELSE resets_at END WHERE resets_at<=? OR count<? RETURNING count",
      key,
      now + 900000,
      now,
      now,
    ...
    if (!entry) throw fail("尝试过于频繁，请十五分钟后再试", 429);
  }
  ...
  async function handle(request, { clientAddress = "local" } = {}) {
    try {
      ...
      if (method === "GET" && path === "/api/health") {
        ...
        return json({ ok: true, service: "Typer Post", storage });
      }
      let data = {};
      if (!["GET", "HEAD"].includes(method)) {
        const origin = request.headers.get("origin");
        if (origin !== url.origin && !allowedOrigins.includes(origin))
      ...
      if (method === "GET" && path === "/api/me") {
        ...
        return json({ user: user ? await publicUser(user) : null });
      }
      if (method === "POST" && ["/api/register", "/api/login"].includes(path)) {
        await throttle(`auth-ip:${digest(clientAddress)}`, 40);
        ...
        await throttle(`auth-handle:${handle}`);
        const found = await one("SELECT * FROM users WHERE handle=?", handle);
        if (path === "/api/login") {
          ...
          await run("DELETE FROM attempts WHERE key=?", `auth-handle:${handle}`);
        ...
        try {
          ...
          if (error.message?.includes("UNIQUE constraint failed: users.handle"))
            throw fail("这个邮址已经有人使用", 409);
          throw error;
        }
        return loginResponse(
          request,
          await one("SELECT * FROM users WHERE id=?", id),
        );
      }
      const user = await session(request);
      ...
      if (path === "/api/logout" && method === "POST") {
        const token = sessionToken(request);
        if (token)
          await run("DELETE FROM sessions WHERE token_hash=?", digest(token));
        return json({ ok: true }, 200, bearerSessions ? {} : {
      ...
      if (path === "/api/settings" && method === "PATCH") {
        ...
        return json({
          user: await publicUser(await one("SELECT * FROM users WHERE id=?", user.id)),
        });
      }
      if (path === "/api/recipient" && method === "GET") {
        await throttle(`lookup:${user.id}`, 100);
      ...
      if (path === "/api/blocks" && ["POST", "DELETE"].includes(method)) {
        ...
        if (method === "POST")
          await run("INSERT OR IGNORE INTO blocks VALUES(?,?)", user.id, other.id);
        else
          await run(
      ...
      if (path === "/api/letters" && method === "POST") {
        ...
        const id = randomUUID(), now = clock(), day = dayFor(user);
        // The quota check, recipient policy and insertion are a single SQLite
        // statement. This stays atomic across concurrent Workers and retries.
        const inserted = await one(
          `INSERT INTO letters(id,sender,recipient,title,snapshot,created_at,deliver_at,sender_day,nonce,fingerprint)
           SELECT ?,s.id,r.id,?,?,?,?+MAX(?,r.min_hours)*3600000,?,?,?
           FROM users s,users r
           WHERE s.id=? AND r.handle=? AND r.id<>s.id AND r.accept_mail=1
             AND NOT EXISTS(SELECT 1 FROM blocks b WHERE b.owner=r.id AND b.blocked=s.id)
             AND (SELECT COUNT(*) FROM letters l WHERE l.sender=s.id AND l.sender_day=?)<s.daily_limit
           ON CONFLICT(sender,nonce) DO NOTHING RETURNING id`,
          id, title, JSON.stringify(snapshot), now, now, hours, day,
          data.nonce, fingerprint, user.id, address, day,
        );
        const letter = await one(`${joined} WHERE l.sender=? AND l.nonce=?`, user.id, data.nonce);
        if (letter) {
          if (letter.fingerprint !== fingerprint)
            throw fail("这封信已经投邮，内容不能再更换", 409);
          return json({ letter: brief(letter, user), user: await publicUser(user) }, inserted ? 201 : 200);
        }
        const available = await one(
          `SELECT r.id FROM users r WHERE r.handle=? AND r.id<>? AND r.accept_mail=1
           AND NOT EXISTS(SELECT 1 FROM blocks b WHERE b.owner=r.id AND b.blocked=?)`,
          address, user.id, user.id,
        );
        if (!available) throw fail("这个邮址目前无法收信", 404);
      ...
      if (match && (method === "GET" || (method === "POST" && match[2]))) {
        const row = await one(`${joined} WHERE l.id=?`, match[1]);
        if (
          !row ||
          (row.sender !== user.id &&
