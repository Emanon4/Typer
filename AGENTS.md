...
- The photograph-clipping mechanical layer is a visual concept only and must not be extended. The production machine is a Blender-authored, separately articulated GLB rendered in the browser from one scene graph and one mechanical state machine.
- Build and validate one complete typewriter before adding more styles. The first gate is one key-to-typebar-to-paper-to-escapement causal chain with a shared carriage, platen, bail, paper, and strike coordinate system.
- Freeze the visible design of the first machine to `public/assets/typewriter-base.png` and `public/assets/typewriter-with-paper.png`. Their low black-enamel body, brass trim, compact key field, front silhouette, platen height, paper position, and overall proportions are the exterior source of truth for Blender modeling.
- Historical typewriters may inform hidden pivots and mechanical causality only. Do not copy an Underwood exterior, expose extra historical machinery, enlarge the typebar basket, or redesign the first machine around a reference that conflicts with the existing Typer artwork.
- During mechanical development, reference images may appear in Blender as non-rendering calibration planes. They must remain in a `REFERENCE_ONLY` collection and must never be exported in the production GLB.
